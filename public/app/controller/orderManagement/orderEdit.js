app.registerCtrl('orderEdit', function($scope, $routeParams, $http, $location, $timeout, $rootScope) {
    // default
    ...
    $scope.$watch('order.customerData.email', function (val) {
        // pre-populate form if email are exist inside database
        if(val){
           $scope.searchByEmailOrPhone(); 
        }
     });
     $scope.$watch('order.customerData.phone', function (val) {
        // pre-populate form if email are exist inside database
        if(val){
           $scope.searchByEmailOrPhone(); 
        }
    ...
    $scope.totalCartCost = function(){
        var total = _.sumBy($scope.order.cart,'subtotal');
    ...
    $scope.addToCart = function(obj){
        ...
        //var found = _.findIndex($scope.order.cart,{'id':obj.id}) // try to lookup the product inside the collection
        //if(found<0){
            // means not found, and push the cart item into the collection
        $scope.order.cart.push(cartItem);
        //}else{
            // otherwise, add the quantity
        //    $scope.order.cart[found].qty += 1;
        //    $scope.order.cart[found].subtotal = ($scope.order.cart[found].price*$scope.order.cart[found].qty);
        //}
        $scope.totalCartCost();
    }
    // force empty To Zero
    $scope.forceZero = function(index){
        if($scope.order.cart[index].price==''){
            $scope.order.cart[index].price = 0;
        }
        if($scope.order.cart[index].qty==''){
            $scope.order.cart[index].qty = 0;
        }
        if($scope.order.cart[index].shipping_cost==''){
            $scope.order.cart[index].shipping_cost = 0;
        }
        // we are also use force zero to repopulate subtotal of it's nested field
        $scope.order.cart[index].subtotal = ($scope.order.cart[index].qty*$scope.order.cart[index].price)+$scope.order.cart[index].shipping_cost;
        $scope.totalCartCost();
    }
    // remove cart
    $scope.removeCart = function(index){
        ...
        var delegated_cart = $scope.order.cart;
        ...
        $scope.order.cart = delegated_cart;
        //$scope.order.cart.splice(index,1);
        $scope.totalCartCost();
        // reset delegator
    ...
    $http.get('/api/orders/get/' + $routeParams.id).then(function(response) {
        ...
        if(!exist){
            ...
            if(response.data.order.status!='unapproved'){
    ...
    $scope.saveOrder = function() {
        swal({
            title: "Are you sure ? ",
            text: "You are going to update an order, are you sure want to continue ?",
            type: "warning",
            showCancelButton: !0,
            closeOnConfirm: !1,
     ...
     $scope.temp_cities = [];
     $scope.$watch('order.cart',function(newcart,oldcart){
     ...
     $scope.createWatcher = function(index){
         // country watcher
         $scope.$watch('order.cart['+index+'].country',function(v,o){
         ...
         // province watcher
         $scope.$watch('order.cart['+index+'].province',function(v){
             if(v && !$scope.delegator){
