app.registerCtrl('inquiryCreate', function($scope, $routeParams, $http, $location, $timeout, $rootScope) {
    // default
    ...
    // default type
    $scope.inquiry.customerData.type = 0;
    $scope.types = [{
        id : 0,
        name : 'Personal'
    },{
    ...
    $http.get('/api/company-types/all').then(function(response) {
        ...
        if($scope.allCompanyType.length<1){
            toastr.error('You have to create at least one company type in order to able create a new inquiry', "Wait a minute!");
    ...
    $scope.saveInquiry = function() {
        swal({
            title: "Are you sure ? ",
            text: "You are going to save your inquiry request, are you sure want to save it ?",
            type: "warning",
            showCancelButton: !0,
            closeOnConfirm: !1,
    ...
    $scope.createWatcher = function(index){
        // country watcher
        $scope.$watch('inquiry.cart['+index+'].country',function(v){
            if(v && !$scope.delegator){
                console.log('delegator tembus , country watcher, index : '+index);
                console.log($scope.delegator);
